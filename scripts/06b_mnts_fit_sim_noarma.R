...
suppressPackageStartupMessages({
  ...
  if (!requireNamespace("temStaR", quietly = TRUE)) {
    remotes::install_github("aaron9011/temStaR-v0.90")
  }
  
  # CDF() now lives in spatstat.univar on modern R
  core_pkgs <- c("spatstat.univar", "data.table", "readr", "yaml", "Matrix")
  for (p in core_pkgs) {
  ...
  library(temStaR)
  library(spatstat.univar)   # gives you CDF()
  library(data.table)
  library(readr)
  library(yaml)
